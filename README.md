...
## 🔮 Модуль универсального ИИ-парсера данных (Smart Credit Parser)

Модуль smart_credit_parser обеспечивает автономную динамическую инжестию неструктурированных и полуструктурированных клиентских файлов (CSV, XLSX, JSON, XML, TXT, PDF) с привязкой к реляционной структуре schema.sql:

- **Динамический разбор schema.sql без хардкода**: автоматическое извлечение 13 таблиц, типов, первичных/внешних ключей, CHECK-ограничений и допустимых ENUM-значений.
- **Fast-path для эталонных файлов**: мгновенное сопоставление со 100% уверенностью без расхода квот ИИ.
- **Мультиязычный ИИ-маппинг**: интеллектуальное сопоставление синонимов на русском, румынском/молдавском и английском языках с оценкой уверенности.
- **Кэширование маппингов**: персистентный SHA-256 кэш структур для ускорения повторного импорта.
- **Транзакционный Dry-Run и валидация**: проверка уникальности, внешних ключей и допустимых диапазонов до фиксации в БД.
- **Ролевая модель (RBAC)**:
  - ANALYST: загрузка, разбор, сухой прогон (запись в БД заблокирована).
  - UNDERWRITER / ADMIN: полный цикл с фиксацией в целевой базе данных.

### Быстрый запуск автономного веб-мастера
`ash
python run_server.py
# Веб-интерфейс мастера доступен по адресу: http://127.0.0.1:8000
`
